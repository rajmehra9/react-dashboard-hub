...
export function ipv4Reason(ip: string): string {
  ...
  if (parts.some((part) => Number(part) > 255)) return "each octet must be between 0 and 255";
...
export function countRecordsInZone(records: Route53RecordItem[], zoneName: string): number {
  return records.filter((record) => record.hosted_zone_name === zoneName).length;
}

export function filterHostedZones(zones: HostedZone[], search: string): HostedZone[] {
