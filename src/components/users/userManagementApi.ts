...
export function normalizeUser(u: RawUserApiResponse): User {
  ...
  return {
    ...
    status,
    maxVMs: u.max_vms,
    maxBuckets: (u as any).max_buckets ?? 0,
    maxVpcs: (u as any).max_vpcs ?? 0,
    maxLoadBalancers: (u as any).max_load_balancers ?? 0,
    maxRdsClusters: (u as any).max_rds_clusters ?? 0,
    maxEksClusters: (u as any).max_eks_clusters ?? 0,
    maxDnsRecords: (u as any).max_dns_records ?? 0,
